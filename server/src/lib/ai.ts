...
export async function generateTrainingPlan(
  ...
  const openai = new OpenAI({
    ...
    defaultHeaders: {
      "HTTP-Referer": process.env.BASE_URL || "http://localhost:3010",
  ...
  function formatPlanResponse(
    ...
    const overview =
      aiResponse.overview && typeof aiResponse.overview === "object"
        ? aiResponse.overview
        : {}
    ...
    const plan: Omit<TrainingPlan, 'id' | 'userId' | 'version' | 'createdAt'>  = {
      ...
      progression:
        ...
        "Increasing weight by 2.5-5lbs when you can complete all sets with good form. Track your progress weekly."
    }
  ...
  function buildPrompt(userProfile: UserProfile): string {
    ...
    return `Create a personalized ${userProfile.days_per_week}-day per week training plan for someone with the following profile:
      ...
      Generate a complete training paln in JSON format with this exact structure:
